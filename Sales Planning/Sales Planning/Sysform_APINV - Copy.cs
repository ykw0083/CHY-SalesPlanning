    ...
    class Sysform_APINV
    {
        ...
        public static void processDataEventafter(SAPbouiCOM.Form oForm, ref SAPbouiCOM.BusinessObjectInfo BusinessObjectInfo)
        {
            ...
            try
            {
                switch (BusinessObjectInfo.EventType)
                {
                    case SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD:
                        ...
                        UserForm_FT_GENSPERR.PurchaseInvoice(xmlDoc.InnerText);
                        ...
                        rs.DoQuery("select T4.U_PRebPer, T4.U_PRebAmt,T0.docnum, T0.docdate,T5.U_Type, T3.quantity, T3.LineNum from OPCH T0 " +
                            ...
                            " where T0.CANCELED = 'N' and T0.docentry = " + xmlDoc.InnerText + " group by T4.U_PRebPer, T4.U_PRebAmt,T0.docnum, T0.docdate,T5.U_Type,T3.quantity, T3.LineNum");
                        if (rs.RecordCount > 0)
                        {
                            ...
                            if (rebAmt > 0)
                            {
                                ...
                                {
                                    lineQty = double.Parse(rs.Fields.Item("lineQty").Value.ToString());
                                    finalAmt = Math.Round(lineQty / totalQty * rebAmt,2);
                                    //if (cnt > 1)
                                    //{
                                    //    oJE.Lines.Add();
                                ...
                                if (retcode != 0)
                                {
                                    ...
                                    rs.DoQuery("update [@FT_SPERRLOG] set U_ErrMsg = '" + SAP.SBOCompany.GetLastErrorDescription() + "' where U_APINVNo = '" + docnum + "'");
                                ...
                                rs.DoQuery("update [@FT_SPERRLOG] set Status = 'C', Canceled = 'Y' where U_APINVNo = '" + docnum + "'");
                                xmlDoc = null;
                            }
                        }
                        break;
            ...
            catch (Exception ex)
            {
                ...
                rs0.DoQuery("update [@FT_SPERRLOG] set U_ErrMsg = '" + ex.Message + "' where U_APINVNo = '" + docnum + "'");
